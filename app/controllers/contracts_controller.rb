class ContractsController < ApplicationController
 ...
 def index
   ...
   @contracts = Contract.where("customer_id" + " LIKE :range", :range => "%" +  params[:customer] + "%").paginate(:page => params[:page], :per_page => 3)
    elsif ! params[:id].nil?
   @contracts = Contract.where("id" + " LIKE :range", :range => "%" +  params[:id] + "%").paginate(:page => params[:page], :per_page => 3)
    else
